...
class Options_Sensor(Proto_Macromodel):
    def __init__(self, model_folder, model_name, info_flow, force_size, proprio_size, action_dim, num_options, device = None):
        super().__init__()
        ...
        self.macro_action = 3 * self.pos_size
    ...
    def get_data(self, states, forces, actions, macro_action, model_tuple, batch_size, sequence_size, padding_masks = None):
        ...
        return options_logits, seq_encs

    def get_logits(self, proprio_diffs, contact_diffs, forces, actions, macro_action, peg_type, padding_masks = None):
        ...
        seq_encs = torch.zeros((batch_size, self.state_size)).float().to(options_logits.device)

        for i in range(self.num_options):
            peg_ol = peg_type[:,i].unsqueeze(1).repeat_interleave(options_logits.size(1), dim=1)
            peg_se = peg_type[:,i].unsqueeze(1).repeat_interleave(self.state_size, dim=1)

            options_logits_i, seq_encs_i = self.get_data(states, forces_reshaped, actions, macro_action,\
             self.ensemble_list[i], batch_size, sequence_size, padding_masks)

            options_logits += peg_ol * options_logits_i
            seq_encs += peg_se * seq_encs_i
    ...
    def forward(self, input_dict):
        ...
        proprio_diffs_comp = input_dict["proprio_diff_comp"].to(self.device)
        actions_comp = input_dict["action_comp"].to(self.device)
        forces_comp = input_dict["force_hi_freq_comp"].to(self.device).transpose(2,3)
        contact_diffs_comp = input_dict["contact_diff_comp"].to(self.device)
        peg_type_comp = input_dict["peg_type_comp"].to(self.device)
        macro_action_comp = input_dict["macro_action_comp"].to(self.device)

        assert torch.sum(torch.where(peg_type == peg_type_comp, torch.zeros_like(peg_type), torch.ones_like(peg_type))) == 0, "All peg types don't match"

        padding_masks = input_dict["padding_mask"].to(self.device)

        options_logits, seq_encs = self.get_logits(proprio_diffs, contact_diffs,\
         forces, actions, macro_action, peg_type, padding_masks)
        
        options_logits_comp, seq_encs_comp = self.get_logits(proprio_diffs_comp, contact_diffs_comp,\
         forces_comp, actions_comp, macro_action_comp, peg_type_comp, padding_masks)

        distances = (seq_encs - seq_encs_comp).pow(2).sum(-1).pow(0.5)
        ...
        probs_comp = F.softmax(options_logits_comp, dim = 1)

        hole_est_comp = torch.zeros_like(probs_comp)
        hole_est_comp[torch.arange(hole_est_comp.size(0)), probs_comp.max(1)[1]] = 1.0

        return {
            'options_class': options_logits,
            'options_class_comp': options_logits_comp,
            'distances': distances,
            'hole_est': hole_est,
...
class Options_Predictor(Proto_Macromodel):
        ...
        for i in range(self.num_options):
            for j in range(self.num_options):
                self.model_dict[(i,j)] = (\
                    ...
                    3 * self.pos_size, self.z_dim, 1, device = self.device).to(self.device),\
    ...
    def get_pred(self, macro_action, peg_type, hole_type):
        options_logits = torch.zeros_like(macro_action[:, :self.num_options])
    ...
    def forward(self, input_dict):
        ...
        macro_action_comp = input_dict["macro_action_comp"].to(self.device)
        peg_type_comp = input_dict["peg_type_comp"].to(self.device)
        hole_type_comp = input_dict["hole_type_comp"].to(self.device)

        options_logits, conf_logits = self.get_pred(macro_action, peg_type, hole_type)
        options_logits_comp, conf_logits_comp = self.get_pred(macro_action_comp, peg_type_comp, hole_type_comp)

        return {
            'options_class': options_logits,
            'conf_class': conf_logits,
            'options_comp_class': options_logits_comp,
...
class Feeling_Distance(Proto_Macromodel):
    def __init__(self, model_folder, model_name, info_flow, pose_size, num_options, device = None):
        super().__init__()
        ...
        for i in range(self.num_options):
            for j in range(self.num_options):
                for k in range(j, self.num_options):
                    if j == k:
                        continue
                    self.model_dict[(i,j,k)] = (\
                        FCN(save_folder + "_expand_state" + str(i) + str(j) + str(k), load_folder + "_expand_state" + str(i) + str(j) + str(k),\
                        2 * 3 * self.pos_size, self.z_dim, 1, device = self.device).to(self.device),\
                         ResNetFCN(save_folder + "_distance_pred" + str(i) + str(j) + str(k), load_folder + "_distance_pred" + str(i) + str(j) + str(k),\
    ...
    def get_pred(self, macro_action, macro_action_comp, peg_type, hole_type, hole_type_comp):
        distances = torch.zeros_like(macro_action[:, 0])

        for key in self.model_dict.keys():
            expand_state, distance_pred = self.model_dict[key]
            i, j, k = key #peg, hole, hole_comp
            peg_bool = peg_type[:,i]
            hole_bool0 = hole_type[:,j]
            hole_comp_bool0 = hole_type_comp[:,k]

            hole_bool1 = hole_type[:,k]
            hole_comp_bool1 = hole_type_comp[:,j]

            estate = expand_state(torch.cat([macro_action, macro_action_comp], dim = 1))

            distances += torch.abs(peg_bool * (hole_bool0 * hole_comp_bool0 + hole_bool1 * hole_comp_bool1) * distance_pred(estate).squeeze())
    ...
    def forward(self, input_dict):
        ...
        distances = self.get_pred(macro_action, macro_action_comp, peg_type, hole_type, hole_type_comp)

        return {
            'distances': distances,
        }

    def distances(self, peg_type, macro_action):
        hole_type0 = torch.zeros_like(peg_type)
        hole_type1 = torch.zeros_like(peg_type)
        hole_type2 = torch.zeros_like(peg_type)

        hole_type0[:,0] = 1.0
        hole_type1[:,1] = 1.0
        hole_type2[:,2] = 1.0

        distances = torch.zeros_like(peg_type)

        distances[:,0] = self.get_pred(macro_action, macro_action, peg_type, hole_type0, hole_type1)
        distances[:,1] = self.get_pred(macro_action, macro_action, peg_type, hole_type0, hole_type2)
        distances[:,2] = self.get_pred(macro_action, macro_action, peg_type, hole_type1, hole_type2)

        return distances

class Insertion_Predictor(Proto_Macromodel):
    def __init__(self, model_folder, model_name, info_flow, pose_size, num_options, device = None):
        super().__init__()
        ...
        for i in range(self.num_options):
            self.model_dict[(i,i)] = (\
                FCN(save_folder + "_expand_state" + str(i), load_folder + "_expand_state" + str(i),\
                3 * self.pos_size, self.z_dim, 1, device = self.device).to(self.device),\
                 ResNetFCN(save_folder + "_insert_pred" + str(i), load_folder + "_insert_pred" + str(i),\
                self.z_dim, 1, self.nl, device = self.device).to(self.device)\
                 )
    ...
    def get_pred(self, macro_action, peg_type, hole_type):
        insert_logits = torch.zeros_like(peg_type[:,0].unsqueeze(1))

        for key in self.model_dict.keys():
            i, j = key
            expand_state, insert_pred = self.model_dict[key]
            peg_bool = peg_type[:,i].unsqueeze(1)
            hole_bool = hole_type[:,j].unsqueeze(1)
            insert_logits += peg_bool * hole_bool * insert_pred(expand_state(macro_action))

        return insert_logits
    ...
    def forward(self, input_dict):
        ...
        insert_logits = self.get_pred(macro_action, peg_type, hole_type)

        return {
            'insert_class': insert_logits,
        }

    def process(self, macro_action, peg_type, hole_type):
        insert_logits = self.get_pred(macro_action, peg_type, hole_type)
