...
class Options_ClassifierTransformer(Proto_Macromodel):
    ...
    def get_data(self, state, forces, model_tuple, batch_size, sequence_size, padding_masks, lengths):
    ...
    def get_logits(self, proprio_diffs, contact_diffs, forces, actions, peg_type, padding_masks, lengths):
        ...
        options_logits01, origin_mean01, origin_cov01 = self.get_data(prestate_reshaped, forces_reshaped, self.ensemble_list[0],\
         batch_size, sequence_size, padding_masks, lengths)
        # options_logits02, origin_mean02, origin_cov02 = self.get_data(prestate_reshaped, forces_reshaped, self.ensemble_list[1],\
        #  batch_size, sequence_size, padding_masks, lengths)
        # options_logits03, origin_mean03, origin_cov03 = self.get_data(prestate_reshaped, forces_reshaped, self.ensemble_list[2],\
        #  batch_size, sequence_size, padding_masks, lengths)
        options_logits11, origin_mean11, origin_cov11 = self.get_data(prestate_reshaped, forces_reshaped, self.ensemble_list[1],\
         batch_size, sequence_size, padding_masks, lengths)
        # options_logits12, origin_mean12, origin_cov12 = self.get_data(prestate_reshaped, forces_reshaped, self.ensemble_list[4],\
        #  batch_size, sequence_size, padding_masks, lengths)
        # options_logits13, origin_mean13, origin_cov13 = self.get_data(prestate_reshaped, forces_reshaped, self.ensemble_list[5],\
        #  batch_size, sequence_size, padding_masks, lengths)

        options_logits21, origin_mean21, origin_cov21 = self.get_data(prestate_reshaped, forces_reshaped, self.ensemble_list[2],\
         batch_size, sequence_size, padding_masks, lengths)
        # options_logits22, origin_mean22, origin_cov22 = self.get_data(prestate_reshaped, forces_reshaped, self.ensemble_list[7],\
        #  batch_size, sequence_size, padding_masks, lengths)
        # options_logits23, origin_mean23, origin_cov23 = self.get_data(prestate_reshaped, forces_reshaped, self.ensemble_list[8],\
        #  batch_size, sequence_size, padding_masks, lengths)
        ...
        # origin_cov2 = oc_peg0 * origin_cov02 + oc_peg1 * origin_cov12 + oc_peg2 * origin_cov22
        # origin_cov3 = oc_peg0 * origin_cov03 + oc_peg1 * origin_cov13 + oc_peg2 * origin_cov23

        return options_logits1, origin_mean1, origin_cov1
    ...
    def forward(self, input_dict):
        ...
        # print(lengths.size())
        # prev_time = time.time()
        options_logits, origin_mean, origin_cov = self.get_logits(proprio_diffs, contact_diffs, forces, actions, peg_type, padding_masks, lengths)
        # print(options_logits.size())
        # print(origin_mean.size())
